...
ofxNDIRecvAVSyncAudio::~AVSyncAudio()
{
	unsubscribe_();
	if(av_sync_) {
		NDIlib_framesync_destroy(av_sync_);
	}
}
void ofxNDIRecvAVSyncAudio::setup(ofxNDIReceiver &receiver)
{
	Threading::setup(receiver);
	av_sync_ = NDIlib_framesync_create(receiver.getInstance());
}
...
{
	std::lock_guard<std::mutex> lock(source_mtx_);
	return false; // NDIlib_framesync_synchronize(av_sync_, source_frame_, &frame_.back()) >= NDIlib_framesync_ret_success;
}
void ofxNDIRecvAVSyncAudio::freeFrame(ofxNDI::AudioFrame &frame)
{
	NDIlib_framesync_free_audio(av_sync_, &frame);
}
